...
def run_one_workload(
    ...
    predictions = pd.read_csv(
        model.lower() + "_results" + "/" + dataset + "/" + preds_file + ".csv"
    ).values.tolist()
    test_file = "test/" + dataset + "/" + test_file
...
def create_plots(dataset, experiment):
    ...
    plt.tight_layout()
    plt.savefig(
        "experiments/" + dataset + "/Unfairness_experiment" + experiment + ".png",
        bbox_inches="tight",
        dpi=200,
    )
    plt.show()
...
def dataset_experiments(
    dataset, sens_att, test_file, preds_file, threshold
